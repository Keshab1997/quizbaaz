// Bangla (bn) string catalogue. Mirrors the keys in `strings_en.dart`.
// Any key omitted here silently falls back to the English value.

const Map<String, String> kStringsBn = {
  // ---------------------------------------------------------------- common --
  'appTitle': 'কুইজবাজ ৩ডি',
  'cancel': 'বাতিল',
  'save': 'সেভ করুন',
  'close': 'বন্ধ করুন',
  'ok': 'ঠিক আছে',
  'retry': 'আবার চেষ্টা করুন',
  'loading': 'লোড হচ্ছে...',
  'pleaseWait': 'অপেক্ষা করুন...',
  'all': 'সব',
  'items': 'আইটেম',
  'today': 'আজ',
  'yesterday': 'গতকাল',
  'daysAgo': '{n} দিন আগে',
  'points': 'পয়েন্ট',
  'seconds': 'সে',
  'coins': 'কয়েন',
  'gems': 'জেম',
  'score': 'স্কোর',
  'correct': 'সঠিক',
  'wrong': 'ভুল',
  'skipped': 'বাদ দেওয়া',
  'time': 'সময়',
  'accuracy': 'নির্ভুলতা',
  'you': 'আপনি',
  'newLabel': 'নতুন',
  'owned': 'কেনা আছে',
  'locked': 'লক করা',
  'active': 'চালু',
  'unlock': 'আনলক করুন',
  'goToShop': 'শপে যান',
  'quantity': 'পরিমাণ',
  'cost': 'মূল্য',
  'currency': 'মুদ্রা',
  'item': 'আইটেম',

  // ------------------------------------------------------------ navigation --
  'navHome': 'হোম',
  'navExplore': 'এক্সপ্লোর',
  'navRanking': 'র‍্যাঙ্কিং',
  'navProfile': 'প্রোফাইল',

  // ------------------------------------------------------------- greetings --
  'greetStillAwake': 'এখনও জেগে আছেন',
  'greetMorning': 'শুভ সকাল',
  'greetAfternoon': 'শুভ দুপুর',
  'greetEvening': 'শুভ সন্ধ্যা',
  'greetNight': 'শুভ রাত্রি',

  // ------------------------------------------------------------- dashboard --
  'dashPlayYourWay': 'নিজের মতো খেলুন',
  'dashQuickActions': 'দ্রুত অ্যাকশন',
  'dashSeeAll': 'সব দেখুন',
  'dashLiveToday': 'দৈনিক কুইজ',
  'dashLeaderboard': 'দৈনিক কুইজ লিডারবোর্ড',
  'dashViewFull': 'সম্পূর্ণ দেখুন',
  'dashDailyChampion': 'দৈনিক কুইজ',
  'dashDailyChampionTitle': 'দৈনিক কুইজ চ্যাম্পিয়ন',
  'dashExploreMore': 'আরও দেখুন',
  'dashMoreOptions': 'আরও অপশন',
  'dashDayStreak': 'দিনের ধারা',
  'dashStreakTitle': "🔥 দৈনিক কুইজ খেলো, স্ট্রিক বাঁচাও",
  
  // ---- streak dialogs
  'streakMotivationTitle': 'তোমার স্ট্রিক বাঁচাও!',
  'streakMotivationDaysToGoal': 'বোনাস পুরস্কার আনলক করতে {n} দিন বাকি!',
  'streakMotivationGoalReached': '🎉 গোলে পৌঁছেছ! বোনাস আনলক!',
  'streakMotivationStart': 'আজ থেকেই শুরু করো! প্রতিটি স্ট্রিক ১ দিনে শুরু হয় 💪',
  'streakMotivationBeginner': 'চমৎকার শুরু! আগুন জ্বালিয়ে রাখো 🔥',
  'streakMotivationBuilding': 'তুমি আগুনে! গোলে পৌঁছাতে আর {n} দিন! 🔥',
  'streakMotivationConsistent': 'অসাধারণ নিয়মিত! {n} দিন ধারে! 🏆',
  'streakMotivationMaster': 'অবিশ্বাস্য! তুমি স্ট্রিক মাস্টার! 👑',
  'streakMotivationLegend': 'কিংবদন্তি! {n} দিন - তুমি অপ্রতিরোধ্য! 🌟',
  'streakMotivationBtn': 'চালিয়ে যাও!',
  'streakMotivationBtnStart': 'আজ থেকে শুরু',
  'streakResetTitle': 'স্ট্রিক হারিয়েছো!',
  'streakResetEncourage30': 'তোমার {n}-দিনের অসাধারণ স্ট্রিক ছিল! সত্যিই দারুণ। এখন নতুন করে শুরু করো! 💪',
  'streakResetEncourage14': '{n} দিন আগুনে ছিলে! প্রতিটি স্ট্রিক আবার গড়া যায়। রাউন্ড ২ এর জন্য প্রস্তুত? 🔥',
  'streakResetEncourage7': '{n} দিনের দৃঢ় স্ট্রিক! নতুন শুরু করার সেরা সময় এখন। চলো! 🚀',
  'streakResetEncourage3': 'ভালো চেষ্টা! {n} দিন শুধু শুরু। আজ থেকে নতুন স্ট্রিক শুরু করো! ✨',
  'streakResetEncourage0': 'কোনো সমস্যা নেই! প্রতিটি মাস্টার একসময় শিক্ষানবিশ ছিল। কিছু দারুণ গড়ি! 🌟',
  'streakShieldAvailable': '🛡️ স্ট্রিক শিল্ড আছে!',
  'streakShieldUse': 'তোমার {n}-দিনের স্ট্রিক তাৎক্ষণিক ফিরিয়ে আনতে ১টি স্ট্রিক শিল্ড ব্যবহার করো!',
  'streakShieldBtn': 'স্ট্রিক ফিরিয়ে আনো',
  'streakShieldProtect': 'ভবিষ্যতের স্ট্রিক রক্ষা করো!',
  'streakShieldProtectDesc': 'স্ট্রিক শিল্ড নাও যাতে আর কখনো স্ট্রিক না হারাও।',
  'streakShieldGet': 'শিল্ড নাও',
  'streakResetBtn': 'আবার শুরু করি! 🚀',
  'streakResetBtnShield': 'না ধন্যবাদ, নতুন করে শুরু',
  'dashAccuracyShort': 'নির্ভুলতা',
  'dashAdminPanel': 'অ্যাডমিন প্যানেল',
  'dashDailyQuiz': 'দৈনিক কুইজ',
  'dashChapters': 'অধ্যায়',
  'dashHistory': 'ইতিহাস',
  'dashBattle': 'ব্যাটল',
  'dashRewards': 'পুরস্কার',
  'dashShop': 'শপ',

  // ------------------------------------------------------------ daily quiz --
  'quizQuitTitle': 'লাইভ কুইজ ছেড়ে যাবেন?',
  'quizQuitBody':
      'এখন চলে গেলে আজকের স্কোর লিডারবোর্ডে যুক্ত হবে না।',
  'quizQuit': 'ছেড়ে দিন',
  'quizNoFiftyFifty': '৫০-৫০ লাইফলাইন শেষ! শপ থেকে আরও কিনুন। 🛒',
  'quizFiftyFiftyUsed': 'এই প্রশ্নে ৫০-৫০ ইতিমধ্যে ব্যবহার করেছেন।',
  'quizFiftyFiftyBlocked': 'এখন ৫০-৫০ ব্যবহার করা যাবে না।',
  'quizNoFreeze': '+১০ সেকেন্ড ফ্রিজ শেষ! শপ থেকে আরও কিনুন। 🛒',
  'quizFreezeUsed': 'এই প্রশ্নে ফ্রিজ ইতিমধ্যে ব্যবহার করেছেন।',
  'quizFreezeBlocked': 'এখন ফ্রিজ ব্যবহার করা যাবে না।',
  'quizNoSkip': 'স্কিপ শেষ! শপ থেকে আরও কিনুন। 🛒',
  'quizSkipUsed': 'এই প্রশ্নে স্কিপ ইতিমধ্যে ব্যবহার করেছেন।',
  'quizSkipBlocked': 'এখন স্কিপ ব্যবহার করা যাবে না।',
  'quizNoHint': 'হিন্ট শেষ! শপ থেকে আরও কিনুন। 🛒',
  'quizHintUsed': 'এই প্রশ্নে হিন্ট ইতিমধ্যে ব্যবহার করেছেন।',
  'quizHintBlocked': 'এখন হিন্ট ব্যবহার করা যাবে না।',
  'quizNoPoll': 'অডিয়েন্স পোল শেষ! শপ থেকে আরও কিনুন। 🛒',
  'quizPollUsed': 'এই প্রশ্নে পোল ইতিমধ্যে ব্যবহার করেছেন।',
  'quizPollBlocked': 'এখন পোল ব্যবহার করা যাবে না।',
  'quizLoadEyebrow': 'আজকের লাইভ কুইজ',
  'quizLoadTitle': 'তৈরি তো, চ্যাম্পিয়ন?',
  'quizLoadBody': '{n}টা তাজা প্রশ্ন সাজানো হচ্ছে।',
  'quizLoadStepShuffle': 'আজকের প্রশ্ন মেশানো হচ্ছে…',
  'quizLoadStepTimer': 'টাইমার সেট হচ্ছে…',
  'quizLoadStepBoost': 'পাওয়ার-আপ চেক হচ্ছে…',
  'quizLoadStepReady': 'প্রস্তুত হও…',
  'quizLoadMetaQuestions': '{n}টি প্রশ্ন',
  'quizLoadMetaCoins': 'সর্বোচ্চ {n} কয়েন',
  'quizLoadChapterEyebrow': 'অধ্যায় সেট',
  'quizLoadChapterTitle': 'সেট {n} লোড হচ্ছে',
  'quizLoadChapterBody': 'দশটা প্রশ্ন। একই জোশ।',

  // --------------------------------------------------------------- chapter --
  'chapterAllSubjects': 'সব বিষয়',
  'chapterBankTitle': 'অধ্যায় ব্যাংক',
  'chapterCount': '{n}টি অধ্যায়',
  'chapterQuestionCount': '{n}টি প্রশ্ন',
  'chapterShort': 'অধ্যায় {n}',
  'chapterLockedMsg':
      '🔒 এই অধ্যায় আনলক করতে আগের অধ্যায়ের পরীক্ষা শেষ করুন!',

  // ----------------------------------------------------------- leaderboard --
  'lbTitle': 'হল অফ ফেম ও র‍্যাঙ্কিং',
  'lbYourPosition': 'আপনার অবস্থান',
  'lbNoScoresToday': 'আজ এখনও কোনো স্কোর নেই',
  'lbNoScoresBody':
      'দৈনিক কুইজ খেলুন, আপনার স্কোর সঙ্গে সঙ্গে এখানে দেখা যাবে।',
  'lbNoChampions': 'এখনও কোনো চ্যাম্পিয়ন প্রকাশিত হয়নি',
  'lbFetching': 'সর্বশেষ ফলাফল আনা হচ্ছে।',
  'lbReward': 'পুরস্কার: {gift}',
  'lbChampScore': 'স্কোর: {score} পয়েন্ট, {time} সেকেন্ডে',

  // --------------------------------------------------------------- profile --
  'profileTitle': 'আমার প্রোফাইল',
  'profileChangeAvatar': 'অ্যাভাটার বদলান',
  'profileEdit': 'প্রোফাইল এডিট',
  'profileSave': 'প্রোফাইল সেভ করুন',
  'profileDetails': 'প্রোফাইলের তথ্য',
  'profilePerformance': 'পারফরম্যান্স',
  'profileBadges': 'ব্যাজ ও অর্জন',
  'profileNoBadges': 'এখনও কোনো ব্যাজ নেই — কুইজ খেলে আনলক করুন।',
  'profileUsername': 'ইউজারনেম',
  'profileFullName': 'পুরো নাম',
  'profileGender': 'লিঙ্গ',
  'profileEmail': 'ইমেইল',
  'profileChange': 'পরিবর্তন',
  'profileMale': 'ছেলে',
  'profileFemale': 'মেয়ে',
  'profileSelectGender': 'লিঙ্গ নির্বাচন করুন',
  'profileDayStreak': 'দিনের ধারা',
  'profileQuizzesPlayed': 'খেলা কুইজ',
  'profileCorrectAnswers': 'সঠিক উত্তর',
  'profileBestDailyScore': 'সেরা দৈনিক স্কোর',
  'profileBattlesWon': 'জেতা ব্যাটল',
  'profileAvgTime': 'গড় সময় / প্রশ্ন',
  'profileLongestStreak': 'দীর্ঘতম ধারা',
  'profileNameEffect': 'নামের ইফেক্ট',
  'profileNameEffectHint':
      'লিডারবোর্ডে আপনার নামকে বিশেষ চেহারা দিন',
  'profileMyPurchases': 'আমার কেনাকাটা',
  'profileMyPurchasesHint': 'শপ থেকে কেনা সবকিছু দেখুন',
  'profileSettings': 'সেটিংস',
  'profileDarkMode': 'ডার্ক মোড',
  'profileSound': 'সাউন্ড ইফেক্ট',
  'profileVibration': 'ভাইব্রেশন',
  'profileNotifications': 'নোটিফিকেশন',
  'profileAbout': 'সম্পর্কে',
  'profileRateApp': 'অ্যাপ রেট করুন',
  'profileShare': 'বন্ধুদের সাথে শেয়ার করুন',
  'profilePrivacy': 'প্রাইভেসি পলিসি',
  'profileTerms': 'শর্তাবলি',
  'profileVersion': 'সংস্করণ {v}',
  'profileConsentOptions': 'বিজ্ঞাপন সম্মতির বিকল্প',
  'profileSignIn': 'সাইন ইন',
  'profileSignOut': 'সাইন আউট',
  'profileSignOutConfirm':
      'আপনি কি সাইন আউট করতে চান? আপনার লোকাল ডেটা থেকে যাবে।',
  'accountDelete': 'অ্যাকাউন্ট মুছুন',
  'accountDeleteWarning':
      'এতে আপনার অ্যাকাউন্ট, প্রোফাইল, কুইজ ইতিহাস, কয়েন, জেমস, রিওয়ার্ড ও শপ আইটেম স্থায়ীভাবে মুছে যাবে। এটি আর ফেরানো যাবে না।',
  'accountDeleteConfirmLabel': 'নিশ্চিত করতে নিচে DELETE লিখুন।',
  'accountDeleting': 'আপনার অ্যাকাউন্ট মুছে ফেলা হচ্ছে…',
  'accountDeleted': 'আপনার অ্যাকাউন্ট মুছে ফেলা হয়েছে। আপনার সব ডেটা সরিয়ে ফেলা হয়েছে।',
  'accountDeleteFailed': 'অ্যাকাউন্ট মুছে ফেলা যায়নি। আবার চেষ্টা করুন।',
  'accountDeleteReauth':
  'accountDeletedPartial': 'আপনার অ্যাকাউন্ট মুছে ফেলা হয়েছে। বাকি ক্লিনআপ সার্ভারে শেষ হচ্ছে।',
      'নিরাপত্তার জন্য Google দিয়ে আবার সাইন ইন করে চেষ্টা করুন।',
  'accountDeleteLocalOnly': 'এই ডিভাইস থেকে আপনার গেস্ট প্রগ্রেস মুছে যাবে।',
  'linkOpenFailed': 'লিংক খোলা যায়নি। আবার চেষ্টা করুন।',
  'shareMsg': 'QuizBaaz খেলুন — দৈনিক কুইজ, ১v১ ব্যাটল ও রিওয়ার্ড! 🎯',
  'profileSignInPrompt': 'অগ্রগতি সেভ করতে সাইন ইন করুন',
  'profileGuestMode': 'বর্তমানে: গেস্ট মোড',
  'profileRegistered': 'বর্তমানে: নিবন্ধিত ব্যবহারকারী',
  'profileGoogleAccount': 'গুগল অ্যাকাউন্ট',
  'profileConnected': 'সংযুক্ত',
  'profileNotConnected': 'সংযুক্ত নয়',
  'profileUpdated': '✅ প্রোফাইল সফলভাবে আপডেট হয়েছে!',
  'profileAvatarUpdated': '✅ অ্যাভাটার সফলভাবে আপডেট হয়েছে!',
  'profileSignedInGoogle': '🎉 গুগল দিয়ে সাইন ইন হয়েছে!',
  'profileEffectNotOwned': 'আগে শপ থেকে এই ইফেক্টটি কিনুন!',
  'profilePlayerFallback': 'কুইজবাজ প্লেয়ার',

  // ---------------------------------------------------------------- avatar --
  'avatarChoose': 'অ্যাভাটার বাছুন',
  'avatarCurrent': 'বর্তমান অ্যাভাটার',
  'avatarTapToChange': 'বদলাতে নিচের অ্যাভাটারে ট্যাপ করুন',
  'avatarCloudSection': 'ক্লাউড অ্যাভাটার',
  'avatarPremium': '👑 প্রিমিয়াম',
  'avatarAlreadyOwned': 'এই অ্যাভাটারটি ইতিমধ্যে আপনার আছে।',
  'avatarNotEnoughGems': 'এই অ্যাভাটার আনলক করতে যথেষ্ট জেম নেই।',
  'avatarPremiumHint':
      'এটি একটি প্রিমিয়াম অ্যাভাটার। আগে শপ থেকে কিনুন!',
  'avatarUnlockTitle': 'ক্লাউড অ্যাভাটার আনলক: {name}',
  'avatarUnlockBody': '{cost} জেম দিয়ে এই ক্লাউড অ্যাভাটার আনলক করবেন?',
  'avatarUnlocked': '✅ {name} আনলক হয়েছে!',

  // ------------------------------------------------------------------ shop --
  'shopTitle': 'পাওয়ার-আপ শপ',
  'shopAllItems': 'সব আইটেম',
  'shopCloudItems': 'ক্লাউড আইটেম',
  'shopEmptyCategory': 'এই ক্যাটাগরিতে এখনও কোনো আইটেম নেই।',
  'shopItemCount': '{n}টি আইটেম',
  'shopOwnedCheck': 'কেনা আছে ✓',
  'shopNotOwned': 'কেনা নেই',
  'shopOwnedTimes': 'কেনা আছে: x{n}',
  'shopBestValue': 'সেরা অফার',
  'shopUnlockedForever': 'চিরকালের জন্য আনলক!',
  'shopAlreadyOwn': 'এই আইটেমটি ইতিমধ্যে আপনার আছে!',
  'shopNotEnough': 'যথেষ্ট {currency} নেই! কুইজ খেলে আরও আয় করুন।',
  'shopAdded': '+{n} আপনার ইনভেন্টরিতে যোগ হয়েছে',
  'shopCatAll': '🏪 সব',
  'shopCatPowerUps': '🎮 পাওয়ার-আপ',
  'shopCatBoosters': '⚡ বুস্টার',
  'shopCatShields': '🛡️ শিল্ড',
  'shopCatAvatars': '🎨 অ্যাভাটার',
  'shopCatEffects': '✨ ইফেক্ট',
  'shopCatBadges': '🏆 ব্যাজ',
  'shopCatPacks': '🎁 প্যাক',
  'shopCoinsLabel': '{n} কয়েন',
  'shopGemsLabel': '{n} জেম',

  // ------------------------------------------------------ purchase history --
  'purchaseTitle': 'কেনাকাটার ইতিহাস',
  'purchaseNone': 'এখনও কিছু কেনা হয়নি',
  'purchaseNoneFiltered': 'কোনো {filter} কেনাকাটা নেই',
  'purchaseEmptyBody': 'পাওয়ার-আপ ও আইটেম কিনতে শপে যান!',
  'purchaseOrders': 'অর্ডার',
  'purchaseTotalSpent': 'মোট খরচ',
  'purchaseCoinsSpent': 'খরচ হওয়া কয়েন',
  'purchaseGemsSpent': 'খরচ হওয়া জেম',
  'purchasedOn': 'কেনা হয়েছে {date}',

  // --------------------------------------------------------------- rewards --
  'rewardsTitle': 'আমার পুরস্কার ও উপহার',
  'rewardsYourGifts': 'আপনার উপহার ও পুরস্কার',
  'rewardsTapPrize': 'ক্লেইম বা ট্র্যাক করতে পুরস্কারে ট্যাপ করুন।',
  'rewardsNone': 'এখনও কোনো পুরস্কার নেই — দৈনিক কুইজ জিতুন! 🏆',
  'rewardsWinDaily': 'প্রতিদিন কুইজ খেলে পুরষ্কার জিতুন!',
  'rewardsWinDailyBody':
      'ডেইলি কুইজে অংশগ্রহণ করে প্রতিদিন Coins, Gems, শপ আইটেম এবং ব্যাজ জিতে নিন।',
  'rewardsClaimNow': 'এখনই ক্লেইম করুন',
  'rewardsClaimed': 'ক্লেইম হয়েছে ✓',
  'rewardsProcessing': 'প্রক্রিয়াধীন',
  'rewardsProcessingCaps': 'প্রক্রিয়াধীন',
  'rewardsShipped': 'পাঠানো হয়েছে',
  'rewardsShippedCaps': 'পাঠানো হয়েছে',
  'rewardsDelivered': 'পৌঁছে গেছে',
  'rewardsDeliveredCaps': 'পৌঁছে গেছে',
  'rewardsDeliveringTo': 'ডেলিভারি হচ্ছে',
  'rewardsDeliveryStatus': 'ডেলিভারি স্ট্যাটাস',
  'rewardsDeliveryAddress': 'ডেলিভারি ঠিকানা',
  'rewardsDigital': 'ডিজিটাল পুরস্কার',
  'rewardsRedeemCode': 'আপনার রিডিম কোড',
  'rewardsCopyCode': 'কোড কপি করুন',
  'rewardsCodeCopied': 'কোড ক্লিপবোর্ডে কপি হয়েছে! 📋',
  'rewardsRedeemHint': 'কোডটি কপি করে পার্টনার সাইটে রিডিম করুন।',
  'rewardsRedeemMark': 'রিডিম ও ক্লেইম করুন',
  'rewardsRedeemed': '✅ কোড সফলভাবে রিডিম হয়েছে!',
  'rewardsSubmitClaim': 'জমা দিন ও ক্লেইম করুন 🎁',
  'rewardsPhone': 'ফোন নম্বর',
  'rewardsAddress': 'সম্পূর্ণ ঠিকানা (বাড়ি, রাস্তা, এলাকা)',
  'rewardsPin': 'পিন কোড',
  'rewardsErrName': 'আপনার নাম লিখুন',
  'rewardsErrPhone': 'সঠিক ফোন নম্বর লিখুন',
  'rewardsErrAddress': 'আপনার সম্পূর্ণ ঠিকানা লিখুন',
  'rewardsErrPin': 'সঠিক পিন কোড লিখুন',

  // --------------------------------------------------------- quiz  history --
  'historyTitle': 'কুইজের ইতিহাস',
  'historyNone': 'কোনো কুইজ ইতিহাস নেই',
  'historyEmptyBody': 'ফলাফল দেখতে প্রথম কুইজটি খেলুন!',
  'historyTotalScore': 'মোট স্কোর',
  'historyQuizzes': 'কুইজ',
  'historyAvgAccuracy': 'গড় নির্ভুলতা',
  'historyDaily': 'দৈনিক',
  'historyChapter': 'অধ্যায়',
  'historyDailyCaps': 'দৈনিক',
  'historyChapterCaps': 'অধ্যায়',
  'historyPlayedOn': 'খেলা হয়েছে {date}',

  // ----------------------------------------------------------- quiz result --
  'resultCompleted': 'কুইজ শেষ! 🎉',
  'resultSummary': 'দুর্দান্ত! এই হলো আপনার পারফরম্যান্স:',
  'resultTotalScore': 'মোট স্কোর',
  'resultReview': 'উত্তর দেখুন 📖',
  'resultLeaderboard': 'লিডারবোর্ড দেখুন 🏆',
  'resultBackHome': 'হোমে ফিরে যান',
  'resultPerfect': '🔥 পারফেক্ট স্কোর! +১০০ কয়েন বোনাস',
  'resultSaveScore': 'স্কোর সেভ করে লিডারবোর্ডে যোগ দিন!',
  'resultGuestBody':
      'আপনি গেস্ট হিসেবে খেলেছেন। দৈনিক লিডারবোর্ডে স্কোর যুক্ত করতে গুগল অ্যাকাউন্ট দিয়ে সাইন ইন করুন।',
  'resultGoogleSignIn': 'এক ট্যাপে গুগল সাইন-ইন',
  'resultSigningIn': 'সাইন ইন হচ্ছে...',
  'resultSignedIn': '🎉 গুগল দিয়ে সাইন ইন! স্কোর লিডারবোর্ডে সেভ হয়েছে!',
  'reviewTitle': 'উত্তর পর্যালোচনা',
  'reviewNone': 'পর্যালোচনা করার মতো কোনো উত্তর নেই।',
  'reviewYourAnswer': 'আপনার উত্তর',
  'reviewCorrectAnswer': 'সঠিক উত্তর',
  'reviewExplanation': '💡 ব্যাখ্যা',
  'reviewQuestionNo': 'প্রশ্ন {n}',

  // ---------------------------------------------------------------- battle --
  'battlePlayer': 'প্লেয়ার',
  'battleBotCorrect': '{n}টি সঠিক',
  'battleArena': '⚔️ ব্যাটল অ্যারেনা',
  'battleRulesLine': 'সব অধ্যায় থেকে মিশে আসা {n}টি প্রশ্ন — প্রতি ম্যাচে নতুন প্রশ্ন',
  'battleChooseDifficulty': 'কঠিনতা বেছে নিন',
  'battleEasy': '😌 সহজ',
  'battleEasyDesc': 'নিশ্চিন্ত গতি — বট প্রায়ই ভুল করে',
  'battleNormal': '🙂 সাধারণ',
  'battleNormalDesc': 'সামতুল্য — বট আপনার সাথে তাল মেলে',
  'battleHard': '😈 কঠিন',
  'battleHardDesc': 'নিষ্ঠুর — বট প্রায় ভুল করে না',
  'battleStartFinding': '⚔️ প্রতিপক্ষ খুঁজুন ও শুরু করুন',
  'battleSearching': 'প্রতিপক্ষ খোঁজা হচ্ছে…',
  'battleSearchingHint': 'একই difficulty-র খেলোয়াড়ের সাথে ম্যাচ হচ্ছে',
  'battleCancelSearch': 'খোঁজা বাতিল করুন',
  'battleBotMatch': 'বট ম্যাচ',
  'battleLiveMatch': 'লাইভ খেলোয়াড়',
  'battleFoundHint': 'প্রতিপক্ষ পাওয়া গেছে! প্রস্তুত হও…',
  'battleGetReady': 'প্রস্তুত হও…',
  'battleRematch': '⚔️ আবার লড়াই',
  'battleBackHome': 'হোমে ফিরে যান',
  'battleWinTitle': 'আপনি জিতেছেন!',
  'battleDrawTitle': 'ড্র!',
  'battleLoseTitle': 'আপনি হেরেছেন!',
  'battleGuestRestricted': 'গেস্ট হিসেবে লাইভ ১v১ খেলা যায় না। অন্য player-কে চ্যালেঞ্জ করতে সাইন ইন করুন।',
  'battleNoQuestions': 'এই মুহূর্তে কোনো প্রশ্ন নেই — একটু পরে আবার চেষ্টা করুন।',
  'battleOpponentLeft': 'ওই player আর উপলব্ধ নেই।',
  'battleStartFailed': 'ম্যাচটি শুরু করা গেল না। আবার চেষ্টা করুন।',
  'battleMatchmakingBroken': 'ম্যাচমেকিং এই মুহূর্তে বন্ধ। কিছুক্ষণ পরে আবার চেষ্টা করুন।',

  // -------------------------------------------------------------- language --
  'languageTitle': 'ভাষা',
  'languageSubtitle': 'অ্যাপের ইন্টারফেসের ভাষা বেছে নিন',
  'languageAppSection': 'অ্যাপের ভাষা',
  'languageSystemDefault': 'ডিভাইসের ভাষা',
  'languageChanged': '✅ ভাষা পরিবর্তন হয়েছে',
  'languageEnglish': 'English (ইংরেজি)',
  'languageBengali': 'বাংলা',
  'languageHindi': 'हिन्दी (হিন্দি)',


  // ---------------------------------------------------------------- dates --
  'month1': 'জানু',
  'month2': 'ফেব',
  'month3': 'মার্চ',
  'month4': 'এপ্রি',
  'month5': 'মে',
  'month6': 'জুন',
  'month7': 'জুলা',
  'month8': 'আগ',
  'month9': 'সেপ্ট',
  'month10': 'অক্টো',
  'month11': 'নভে',
  'month12': 'ডিসে',

  // -------------------------------------------------------- chapter  sets --
  'setsTitle': 'সেট',
  'setsHistoryTab': 'ইতিহাস',
  'setsSet': 'সেট {n}',
  'setsSetOf': '{total}টির মধ্যে সেট {n}',
  'setsQuestions': '{n}টি প্রশ্ন',
  'setsContinue': 'চালিয়ে যান — সেট {n}',
  'setsStart': 'সেট {n} শুরু করুন',
  'setsAllDone': 'সব সেট শেষ 🎉',
  'setsAllDoneBody': 'নিচে যেকোনো সেট আবার খেলুন, বা নতুন প্রশ্ন যোগ হলে ফিরে আসুন।',
  'setsLocked': 'আনলক করতে সেট {n} শেষ করুন',
  'setsCleared': 'সম্পন্ন',
  'setsBest': 'সেরা {correct}/{total}',
  'setsRetry': 'আবার খেলুন',
  'setsPractice': 'অনুশীলন',
  'setsPracticeNote': 'অনুশীলন — কয়েন বা স্কোর যোগ হবে না',
  'setsPracticeBanner': 'অনুশীলন · কিছুই গণনা হচ্ছে না',
  'setsNoHistory': 'এখনও কোনো সেট শেষ হয়নি',
  'setsNoHistoryBody': 'একটি সেট শেষ করলে সেটি এখানে আসবে, যখন খুশি আবার খেলতে পারবেন।',
  'setsAttempts': '{n} বার খেলা',
  'setsEmpty': 'এই অধ্যায়ে এখনও কোনো প্রশ্ন নেই',
  'setsEmptyBody': 'প্রশ্ন যোগ করা হচ্ছে। শীঘ্রই দেখুন!',
  'setsPlayedOn': 'শেষ খেলা {date}',

  // ------------------------------------------------------ notifications --
  'notifDailyTitle': 'ডেইলি কুইজ লাইভ',
  'notifDailyBody': '১০টা প্রশ্ন। এক রান। লিডারবোর্ডে উঠুন।',
  'notifStreakTitle': 'আপনার স্ট্রিক হুমকির মুখে',
  'notifStreakBody': '{n} দিনের স্ট্রিক — আজকের ডেইলি কুইজ খেলুন, স্ট্রিক বাঁচান।',
  'notifPermissionDenied':
      'নোটিফিকেশন অনুমতি বন্ধ। ডেইলি কুইজ রিমাইন্ডার পেতে সিস্টেম সেটিংসে চালু করুন।',
  'notifBellOn': 'ডেইলি কুইজ রিমাইন্ডার চালু · সন্ধ্যা ৭টা',
  'notifBellOff':
      'রিমাইন্ডার বন্ধ। প্রোফাইল → সেটিংসে নোটিফিকেশন চালু করুন।',
  'notifChannelName': 'দৈনিক রিমাইন্ডার',
  'notifChannelDesc': 'ডেইলি কুইজ ও স্ট্রিক রিমাইন্ডার',
  'notifInboxTitle': 'নোটিফিকেশন',
  'notifInboxNone': 'এখনও কোনো নোটিফিকেশন নেই',
  'notifInboxEmptyBody':
      'ডেইলি কুইজ রিমাইন্ডার ও লাইভ পুশ এখানে দেখা যাবে।',
  'notifInboxClear': 'সব মুছুন',
  'notifInboxClearConfirm':
      'এই ডিভাইস থেকে সব নোটিফিকেশন মুছে ফেলবেন? ফেরানো যাবে না।',
  'notifInboxCleared': 'নোটিফিকেশনের ইতিহাস মুছে গেছে',
  'notifKindReminder': 'রিমাইন্ডার',
  'notifKindPush': 'পুশ',
  'notifJustNow': 'এখনই',
  'notifMinutesAgo': '{n} মি আগে',
  'onboardingSkip': 'এড়িয়ে যান',
  'onboardingNext': 'পরবর্তী',
  'onboardingStart': 'শুরু করুন',
  'onboard1Title': 'প্রতিদিনের লাইভ কুইজ ও ৩ডি স্ট্রিক',
  'onboard1Desc': 'প্রতিদিন ১০টি নতুন প্রশ্ন সমাধান করুন, ফায়ার স্ট্রিক বজায় রাখুন এবং জিতে নিন পয়েন্ট ও এক্সক্লুসিভ রিওয়ার্ড।',
  'onboard2Title': '১v১ লাইভ কুইজ ব্যাটল অ্যারেনা',
  'onboard2Desc': 'বন্ধুদের বা অনলাইন প্রতিপক্ষের সাথে রিয়েল-টাইম কুইজ লড়াইয়ে অংশ নিন এবং লিডারবোর্ডের শীর্ষে উঠুন।',
  'onboard3Title': 'বিষয়ভিত্তিক ব্যাংক ও প্রতিদিনের সেরা পুরস্কার',
  'onboard3Desc': 'অধ্যায়ভিত্তিক কুইজ প্র্যাকটিস করুন, স্টাডি নোট পান এবং প্রতিদিনের বিজয়ী হিসেবে ৩ডি ট্রফি অর্জন করুন।',
  'splashTagline': 'নেক্সট-জেন ৩ডি গ্যামিফাইড লার্নিং প্ল্যাটফর্ম',
};
